...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (accError) {
      console.error("[Sync] Error fetching accounts:", accError.message);
    ...
    console.log(`[Sync] Complete: ${syncedCount} synced, ${errorCount} errors out of ${accounts.length} accounts`);

    return new Response(JSON.stringify({ synced: syncedCount, errors: errorCount, total: accounts.length }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (err) {
